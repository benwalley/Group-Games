...
export default function PlayingGameUpdateForm(props) {
  const {
    id: idProp,
    playingGame,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = playingGameRecord
      ? { ...initialValues, ...playingGameRecord }
      : initialValues;
    setGameId(cleanValues.gameId);
    setGameName(cleanValues.gameName);
    setStartedBy(cleanValues.startedBy);
    setErrors({});
  };
  const [playingGameRecord, setPlayingGameRecord] = React.useState(playingGame);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(PlayingGame, idProp)
        : playingGame;
      setPlayingGameRecord(record);
    };
    queryData();
  }, [idProp, playingGame]);
  React.useEffect(resetStateValues, [playingGameRecord]);
  const validations = {
    gameId: [],
    gameName: [],
    startedBy: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            PlayingGame.copyOf(playingGameRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "PlayingGameUpdateForm")}
      {...rest}
    >
      <TextField
        label="Game id"
